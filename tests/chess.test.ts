...
it('requires the correct rooks for castling',()=>{expect(()=>new ChessGame('4k3/8/8/8/8/8/8/4K3 w KQ - 0 1')).toThrow();});
it('requires the king on its home square for castling',()=>{expect(()=>new ChessGame('4k3/8/8/8/8/8/R6R/8 w KQ - 0 1')).toThrow();});
...
it('exports correct PGN checkmate result',()=>{const g=new ChessGame('7k/6Q1/7K/8/8/8/8/8 b - - 0 1');expect(g.pgn()).toBe('1-0');});
it('exports FEN and Result tags',()=>{const fen='4k3/8/8/8/8/8/4K3/8 w - - 0 1';const g=new ChessGame(fen);const p=g.pgn({Event:'Test'});expect(p).toContain('[SetUp "1"]');expect(p).toContain('[FEN "'+fen+'"]');expect(p).toContain('[Result "1/2-1/2"]');});
...
it('rejects a king move adjacent to the opposing king',()=>{const p=fromFEN('8/8/8/8/8/8/4K1k1/8 w - - 0 1');expect(legalMoves(p).some(m=>m.from===12&&m.to===13)).toBe(false);});
