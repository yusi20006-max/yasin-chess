...
export function chooseMove(p:Position,depth:number):Move|undefined{const opening=bookMove(p);if(opening)return opening;const ms=legalMoves(p);let best:Move|undefined,bestScore=-Infinity;for(const m of ms){const score=-negamax(applyMove(p,m),Math.max(0,depth-1),-Infinity,Infinity);if(score>bestScore){bestScore=score;best=m}}return best}
