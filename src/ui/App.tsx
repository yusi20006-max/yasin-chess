...
export default function App(){
 ...
 const moveFromTo=(from:Square,to:Square)=>{if(!hydrated||thinking||game.position.turn!=='w')return;const piece=game.position.board[from];if(piece?.color!=='w')return;const opts=legalMovesFrom(game.position,from).filter(m=>m.to===to);if(!opts.length)return;if(opts.length>1){setPromotion({moves:opts});return}commitMove(opts[0])};
 const click=(s:Square)=>{if(!hydrated||thinking||game.position.turn!=='w')return;const pc=game.position.board[s];if(selected!==null){const opts=legalMovesFrom(game.position,selected).filter(m=>m.to===s);if(opts.length){if(opts.length>1){setPromotion({moves:opts});return}commitMove(opts[0]);return}}if(pc?.color===game.position.turn)setSelected(s)};
