...
export class ChessGame{
 position:Position; history:{move:Move;san:string;before:Position;after:Position}[]=[]; future:{move:Move;san:string;before:Position;after:Position}[]=[]; keys:string[]; readonly startFEN:string;
 ...
 resultToken(){const s=this.status();if(s==='checkmate')return this.position.turn==='w'?'0-1':'1-0';if(s==='stalemate'||s.startsWith('draw-'))return '1/2-1/2';return '*'}
