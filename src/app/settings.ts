...
export function saveSettings(patch:Partial<AppSettings>):AppSettings{const next:AppSettings={...loadSettings(),...patch,version:1};if(patch.engine)next.engine=normalizeEngineConfig(patch.engine);if(typeof localStorage!=='undefined')localStorage.setItem(KEY,JSON.stringify(next));return next}
